...
* capture list：捕获外部变量列表(值捕获，引用变量， 隐藏捕获)
* params list：形参列表
...
```
<em>注意</em>: 如果以传值方式捕获外部变量，则在Lambda表达式函数体中不能修改该外部变量的值。
...
<em>总结</em>：C++11中的Lambda表达式捕获外部变量主要有以下形式：
...
# Lambda表达式的参数
Lambda表达式的参数和普通函数的参数类似，但在Lambda表达式中传递参数还有一些限制，主要有以下几点：
